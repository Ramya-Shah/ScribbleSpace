  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 max-h-[400px]">
        {messages.length === 0 ? (
          <div className="text-gray-500 text-center py-4">No messages yet. Start chatting!</div>
        ) : (
          messages.map((msg, index) => (
            <div
              key={index}
              className={`${msg.system ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100'} p-2 rounded-lg`}
            >
              {msg.system ? (
